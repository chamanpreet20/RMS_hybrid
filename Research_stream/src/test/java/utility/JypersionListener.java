...
import utility.CaptureScreenshot;
	...
	public class JypersionListener implements ITestListener {
		...
		public void onTestSuccess(ITestResult result) {
			...
			this.successTable.addCell(cell);
			cell = new PdfPCell(new Paragraph(result.getName()+ "-" + Arrays.toString(result.getParameters())));
		...
		public void onTestFailure(ITestResult result) {
			...
			//====================================changed to capture screenshot=================================
	String file=System.getProperty("user.dir")+"\\"+"screenshot"+(new Random().nextInt())+".png";

			try {
				
				File scr=((TakesScreenshot)SelectBrowser.driver).getScreenshotAs(OutputType.FILE);
				FileUtils.copyFile(scr, new File(file));
			
				System.out.println("Screenshot captured for pdf");
			} catch (IOException e) {
				e.printStackTrace();
			}
